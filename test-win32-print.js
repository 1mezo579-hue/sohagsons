...
const tempPsPath = path.join(__dirname, "print-raw.ps1");
fs.writeFileSync(tempPsPath, psScript, "utf8");

exec(`powershell -ExecutionPolicy Bypass -File "${tempPsPath}"`, (error, stdout, stderr) => {
  if (error) {
    console.error("Error:", error.message);
    console.error("Stderr:", stderr);
  } else {
    console.log("Stdout:", stdout);
    console.log("تم إرسال أمر الطباعة RAW (Win32) بنجاح!");
  }
});
